...
class nnUNetTrainerV2CascadeFullRes(nnUNetTrainerV2):
    ...
    def do_split(self):
        ...
        for k in self.dataset:
            self.dataset[k]["seg_from_prev_stage_file"] = join(
                self.folder_with_segs_from_prev_stage, k + "_segFromPrevStage.npz"
            )
        ...
        for k in self.dataset_val:
            self.dataset_val[k]["seg_from_prev_stage_file"] = join(
                self.folder_with_segs_from_prev_stage, k + "_segFromPrevStage.npz"
            )
        for k in self.dataset_tr:
            self.dataset_tr[k]["seg_from_prev_stage_file"] = join(
                self.folder_with_segs_from_prev_stage, k + "_segFromPrevStage.npz"
            )
        ...
        for k in self.dataset_val.keys():
            ...
            if (
                ...
                data_for_net = np.concatenate(
                    (
                        data[:-1],
                        to_one_hot(seg_from_prev_stage[0], range(1, self.num_classes)),
                    )
                )
